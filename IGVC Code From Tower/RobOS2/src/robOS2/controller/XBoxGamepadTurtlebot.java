...
import robOS2.hardware.clearPath.TurtleBot;
...
public class XBoxGamepadTurtlebot {
	static Logger log = Logger.getLogger(XBoxGamepadTurtlebot.class.getName());
	 ...
	 */
	public XBoxGamepadTurtlebot() {
		log.debug("Creating a new XBox Gamepad controller.");
		try {
			Connection conn = new Connection("TurtleBot");
			conn.connectNoParity("COM12", 8, 57600);
			
			_motorController = new TurtleBot(conn);
	...
	private class XBoxGamepadListener implements JoystickListener{
		...
		public void joystickAxisChanged(Joystick arg0) {
			...
			if(_turboMode){
				...
				log.debug("Forward Velocity Set To: " +(int)-(_js.getY() * 100));
				log.debug("Rotational Velocity Set To: " + (int)(_js.getZ() * 100));
			} else {
				...
				log.debug("Forward Velocity Set To: " +(int)-(_js.getY() * SCALE_CONST));
				log.debug("Rotational Velocity Set To: " + (int)(_js.getZ() * (SCALE_CONST * 1.5)));
			}
		}
